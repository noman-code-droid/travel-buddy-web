...
export default function FindRideView({ onClose, onRideClick, onViewDriverProfile }: FindRideViewProps) {
  ...
  return (
    ...
    >
      {/* Header - Matches activity_find_ride.xml Header Layout */}
      ...
      <div className="p-4 space-y-6 flex-1 overflow-y-auto pb-24 no-scrollbar">
        {/* Search Section - Styled like activity_find_ride.xml form */}
        <div className="space-y-4 px-1">
          ...
          <div className="pt-2">
            <Button
              onClick={fetchRides}
              className="android-btn-primary !h-[56px] !rounded-[24px]"
            >
        ...
        <div className="flex justify-between items-center mt-8 px-2">
          <h3 className="text-[14px] font-bold text-white uppercase tracking-[0.15em]">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-[#666666] text-[10px] font-black uppercase tracking-[0.2em]">Locating active routes</p>
          </div>
        ) : filteredRides.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 opacity-20">
             <Navigation className="w-16 h-16 mb-4" />
             <p className="italic text-sm text-white text-center">No rides found matching your route</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredRides.map((ride) => (
              ...
              >
                {/* Card - Matches item_ride_post.xml premium styling */}
                <div className="bg-[#1E1E1E] rounded-[24px] p-6 shadow-xl border border-white/[0.03]">
                  ...
                  {/* Vertical Route Indicator - Matches item_ride_post.xml design */}
                  <div className="flex gap-4 mb-6 px-1">
                    <div className="flex flex-col items-center py-1">
                      <div className="w-2.5 h-2.5 rounded-full bg-[#FFD500] shadow-[0_0_10px_rgba(255,213,0,0.5)]" />
                    ...
                    <div className="flex-1 space-y-4 overflow-hidden">
                      <p className="text-[13px] text-white font-medium truncate">{ride.pickup}</p>
                      <p className="text-[13px] text-white font-medium truncate">{ride.dropoff}</p>
                    </div>
                  </div>

                  <div className="flex justify-between items-end pt-4 border-t border-white/5">
                    <div className="space-y-3">
                      <div className="flex gap-4 text-[11px] font-black text-[#666666] uppercase tracking-wider">
                      ...
                      {/* Status Badge */}
                      <div className="inline-flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-xl border border-white/5">
                         <Check className="w-3 h-3 text-[#FFD500]" strokeWidth={3} />
                         <span className="text-[10px] font-black text-[#FFD500] uppercase tracking-widest">Available</span>
                      </div>
                    </div>

                    <div className="text-right space-y-0.5">
                      <p className="text-[24px] font-black text-[#FFD500] tracking-tighter italic leading-none">RS {ride.price}</p>
                    </div>
                  </div>
      ...
      {/* Route Preview Dialog Integration */}
      <RoutePreviewDialog
        isOpen={!!previewRide}
        onClose={() => setPreviewRide(null)}
        pickup={previewRide ? { lat: previewRide.pickupLat, lng: previewRide.pickupLng } : { lat: 0, lng: 0 }}
        dropoff={previewRide ? { lat: previewRide.dropoffLat, lng: previewRide.dropoffLng } : { lat: 0, lng: 0 }}
      />
    </motion.div>
  );
}
